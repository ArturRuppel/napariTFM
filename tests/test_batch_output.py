...
def test_in_place_when_no_processed_root():
    ...
    assert plan.output_dirs["/data/exp_A"] == Path("/data") / TFM_DATA_BUCKET / "exp_A"
    assert plan.output_dirs["/data/exp_B"] == Path("/data") / TFM_DATA_BUCKET / "exp_B"
...
def test_mirror_tree_under_processed_root():
    ...
    # Longest common parent is /data; tree reproduced relative to it, with the
    # TFM_data bucket sitting where the input data would land in that tree.
    assert plan.output_dirs["/data/cond1/exp_A"] == Path("/out/cond1") / TFM_DATA_BUCKET / "exp_A"
    assert plan.output_dirs["/data/cond2/exp_B"] == Path("/out/cond2") / TFM_DATA_BUCKET / "exp_B"
    assert plan.warnings == []
...
def test_single_folder_falls_back_to_basename():
    ...
    assert plan.output_dirs["/data/cond1/exp_A"] == Path("/out") / TFM_DATA_BUCKET / "exp_A"
    assert plan.warnings == []
...
def test_disconnected_roots_fall_back_to_basenames_with_warning(monkeypatch):
    ...
    assert plan.output_dirs["C:/a/exp_A"] == Path("/out") / TFM_DATA_BUCKET / "exp_A"
    assert plan.output_dirs["D:/b/exp_B"] == Path("/out") / TFM_DATA_BUCKET / "exp_B"
...
def test_experiment_ntfm_path_matches_batch_write_location():
    ...
    # helper; the batch writes to resolve_output_plan's dir + RESULTS_FILENAME.
    # They must be byte-identical or the dots read a file the batch never wrote.
    folder = "/data/cond1/pos_00"

    # In-place mode (no processed_root): TFM_data/ bucket sibling of the folder.
    out_dir = experiment_output_dir(folder, None)
    assert out_dir == Path("/data/cond1") / TFM_DATA_BUCKET / "pos_00"
    assert experiment_ntfm_path(folder, None) == out_dir / RESULTS_FILENAME

    # The batch resolves the same way for the same single folder.
    plan = resolve_output_plan([folder], None)
    assert experiment_ntfm_path(folder, None) == plan.output_dirs[folder] / RESULTS_FILENAME
...
def test_experiment_ntfm_path_honours_processed_root():
    ...
    assert experiment_ntfm_path("/data/cond1/exp_A", "/out") == out_dir / RESULTS_FILENAME
