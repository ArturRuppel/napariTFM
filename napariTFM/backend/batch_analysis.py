...
class BatchAnalysis:
    ...
    def process_folder(self, folder_path: str, output_dir=None) -> None:
        ...
        try:
            ...
            # Masks are supplied externally as an input layer: loaded once from the
            # input folder (never generated by napariTFM) and shared by the stages
            # that use them -- displacement (confine the measurement to the cell +
            # margin) and stress (define the cell regions).
            mask_data = self._load_mask(folder)

            # Handle displacement
            displacement_data = self._handle_displacement_execution(folder, tfm_folder, mask_data)
            ...
            # Handle stress analysis (reuses mask_data loaded above)
    ...
    def _handle_displacement_execution(self, folder: Path, tfm_folder: Path,
                                       mask_data: Optional[np.ndarray] = None) -> Optional[dict]:
        ...
        ``mask_data`` (optional) confines the measurement to the cell + margin when
        ``disp_mask_margin_um`` is finite; it is a no-op otherwise.
        """
        def body():
            ...
            return self._execute_displacement_analysis(tfm_folder, raw, mask=mask_data)
    ...
    def _execute_displacement_analysis(self, tfm_folder: Path, image_data: dict,
                                       mask: Optional[np.ndarray] = None) -> Optional[DisplacementResult]:
        """
