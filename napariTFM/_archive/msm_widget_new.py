...
class MSMWidget(BaseAnalysisWidget):
    ...
    def _get_current_parameters(self) -> MSMParameters:
        """Get current MSM parameters from UI widgets."""
    ...
    def _pre_generate_meshes(self, mask_stack):
        ...
        meshes = []
        density_factor = self.parameter_spins['density_factor'].value()
        algorithm = self.MESH_ALGORITHMS_NUMBERS[self.parameter_combos['algorithm'].currentText()]
        use_optimization = self.parameter_checks['use_optimization'].isChecked()
    ...
    def analyze_all_frames_worker(self):
        ...
        for frame in range(num_frames):
            ...
            analyzer = MonolayerStressMicroscopy(
                mask=self.data_manager.masks[frame],
                density_factor=self.parameter_spins['density_factor'].value(),
                algorithm=self.MESH_ALGORITHMS[self.parameter_combos['algorithm'].currentText()],
                use_optimization=self.parameter_checks['use_optimization'].isChecked(),
    ...
    def preview_current_frame(self):
        """Preview stress calculation for current frame using service layer."""
        try:
            if not self._validate_prerequisites():
                return
    ...
    def preview_mesh(self):
        """Generate and display mesh preview using service layer."""
        try:
            if not self._validate_prerequisites():
                return
            ...
            status_text = (
                ...
                f"{len(preview_result.elements)} elements\n"
                f"Min angle: {preview_result.quality_metrics['min_angle']:.1f}°\n"
                f"Mean quality: {preview_result.quality_metrics['mean_quality']:.3f}"
            )
            self._update_status(status_text, 100)
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.visualization_manager.remove_mesh_visualization()
    ...
    def _create_mask_from_images(self):
        """Create masks from images using service layer."""
        try:
            active_layer = self._get_active_image_layer() or self._get_active_labels_layer()
            if active_layer is None:
                raise ValueError("No active image or label layer found")

            params = self._get_current_parameters()

            # Get target shape from force data if available
            target_shape = None
            downscale_factor = 1
            if self.data_manager.force_field is not None:
                target_shape = self.data_manager.force_field.shape[1:3]
                downscale_factor = self.data_manager.force_params['downscale_factor']
            ...
            self._update_mask_visualization(vis_masks)
            ...
            # Update status
            shape_info = f"{analysis_masks.shape[1]}x{analysis_masks.shape[2]}"
            vis_shape_info = f"{vis_masks.shape[1]}x{vis_masks.shape[2]}"
            self._update_status(
                f"Successfully created {len(analysis_masks)} masks\n"
            ...
            self._handle_error(f"Failed to create masks: {str(e)}")
            self.progress_bar.setValue(0)
