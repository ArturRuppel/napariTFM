...
class DisplacementAnalysisWidget(BaseAnalysisWidget):
    ...
    def __init__(self, viewer: "napari.Viewer", data_manager: "DataManager",
        ...
        # Track whether parameters have been modified
        ...
        # Setup UI (this will populate parameter_spins)
        self._setup_ui()
    ...
    def preview_displacement(self):
        ...
        try:
            ...
            # Get cell data if needed
            cells = None
            if self.show_cells_check.isChecked():
                cell_stack = self.data_manager.preprocessed_cell_stack
                if cell_stack is None:
                    cell_stack = self.data_manager.cell_stack
                if cell_stack is not None:
                    cells = cell_stack[current_frame]
            ...
            self.visualization_manager.update_displacement_visualization(
                ...
                show_magnitude=self.show_magnitude_check.isChecked(),
                vector_stride=self.vector_stride_spin.value()
            )
    ...
    def _on_frame_changed(self, event=None):
        ...
        if self.current_flow is not None and not self.parameters_modified:
    ...
    def _connect_signals(self):
        ...
        # Parameter updates - just mark as modified, don't trigger preview
        for spin in self.parameter_spins.values():
            spin.valueChanged.connect(self._update_parameters)

        # d_max specific update
        self.d_max_spin.valueChanged.connect(self._update_d_max)

        # Visualization updates
        for check in [self.show_overlay_check, self.show_vectors_check,
                      self.show_magnitude_check, self.show_cells_check]:
            check.toggled.connect(lambda: self._update_visualization_if_active(force=True))
        self.vector_stride_spin.valueChanged.connect(lambda: self._update_visualization_if_active(force=True))
    ...
    def _create_parameters_group(self) -> QGroupBox:
        ...
        # Add d_max parameter
        d_max_row = QHBoxLayout()
        d_max_row.addWidget(QLabel("Max Displacement (d_max):"))
        self.d_max_spin = QDoubleSpinBox()
        self.d_max_spin.setRange(0.1, 100.0)
        self.d_max_spin.setValue(10.0)
        self.d_max_spin.setSingleStep(0.1)
        self.parameter_spins['d_max'] = self.d_max_spin
        d_max_row.addWidget(self.d_max_spin)
        layout.addLayout(d_max_row)
    ...
    def analyze_all_frames(self):
        ...
        try:
            ...
            current_frame = self.viewer.dims.current_step[0]
            self.current_flow = results['flows'][current_frame]
            self.preview_displacement()  # This will update visualization
    ...
    def _update_ui_state(self):
        ...
        has_beads = (self.data_manager.preprocessed_bead_stack is not None or
                     self.data_manager.bead_stack is not None)
        has_cells = (self.data_manager.preprocessed_cell_stack is not None or
                     self.data_manager.cell_stack is not None)

        # Update status labels
        if has_beads:
            data = (self.data_manager.preprocessed_bead_stack or
        ...
        if has_reference:
            data = (self.data_manager.preprocessed_reference or
        ...
        if has_cells:
            data = (self.data_manager.preprocessed_cell_stack or
        ...
        self.show_cells_check.setEnabled(has_cells)
    ...
    def _register_controls(self):
        ...
        controls = [
            ...
            self.load_cells_btn,
            self.show_overlay_check,
            self.show_vectors_check,
            self.show_magnitude_check,
            self.show_cells_check,
            self.vector_stride_spin,
            self.preview_btn,
            self.analyze_btn,
    ...
    def _connect_signals(self):
        ...
        for check in [self.show_overlay_check, self.show_vectors_check,
                     self.show_magnitude_check, self.show_cells_check]:
            check.toggled.connect(self._update_visualization_if_active)
        self.vector_stride_spin.valueChanged.connect(self._update_visualization_if_active)

        # Action buttons
        self.preview_btn.clicked.connect(self.preview_displacement)
        self.analyze_btn.clicked.connect(self.analyze_all_frames)

    def _update_visualization_if_active(self):
        """Update visualization if there's active flow data."""
        if self.current_flow is not None:
            self._update_visualization(self.current_flow)
