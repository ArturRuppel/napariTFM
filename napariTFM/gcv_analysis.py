...
class GCVAnalysis:
    ...
    def calculate_gcv(self, u_data: np.ndarray, v_data: np.ndarray,
        ...
        reg_min, minG, G, reg_param = self._gcv_blockdiag(blockU, s, b, lambdarange, plot)

        # Convert to regularization parameter
        kx, ky = self._calculate_wave_vectors(u_data.shape[0])
        kmax = np.sqrt(kx.max() ** 2 + ky.max() ** 2)
        regularization = reg_min / (kmax ** 2)
    ...
    def _gcv_blockdiag(self, U: np.ndarray, s: np.ndarray, b: np.ndarray,
                       lambdarange: np.ndarray, plot: bool) -> tuple:
        """Calculate GCV function and find minimum."""
        G = np.array([self._gcvfun(lam, s ** 2, b, 0, 0) for lam in lambdarange])
        minGi = G.argmin()

        reg_min = fmin(self._gcvfun, x0=lambdarange[minGi],
                       args=(s ** 2, b, 0, 0), disp=0)[0]
        minG = self._gcvfun(reg_min, s ** 2, b, 0, 0)

        if plot:
            import matplotlib.pyplot as plt
            plt.figure()
            plt.plot(lambdarange, G, '-')
    ...
    def _gcvfun(self, lmbda: float, s2: np.ndarray, beta: np.ndarray,
                delta0: float, mn: int) -> float:
        """Calculate GCV function value."""
